...
// From http://www.phpclasses.org/package/1289-PHP-CSS-parser-class.html
// Modified for use with proxy
...
class CSSParser {
  ...
  public function ParseCSS($css) {
    ...
    $this->cssData[$index] = array('__root__' => array());
          ...
          while($idx < strlen($code)) {
            ...
            if($inQuote !== false) {
              ...
              if(preg_match('/^(.*)\[([0-9]*)\]$/Us', $property, $propMatch)) {
                $property = $propMatch[1].'['.self::$propCounter.']';
                self::$propCounter += 1;
              }
      ...
      foreach($this->cssData[$index][$media] as $section => $sectionValues) {
        ...
        foreach($sectionValues as $property => $value) {
          ...
          if(is_array($block) && isset($block[$property])) {
            if(preg_match('/^'.self::ConvertWildcards($block[$property]).'$/Usi', $value) == 0) {
  ...
  public function ExportStyle($index, $block = false, array $medias = null) {
    $result = '';
    if(isset($this->cssData[$index])) {
      foreach($this->cssData[$index] as $media => $mediaValues) {
          if ($medias != null && in_array($media, $medias)) {
              continue;
          }
        if($media != '__root__') {
          $result .= "@media $media {\n";
